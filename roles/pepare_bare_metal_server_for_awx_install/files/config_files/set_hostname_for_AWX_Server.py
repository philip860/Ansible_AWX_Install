...
subprocess.check_output([f"sed -i 's/ITS-SYSVAR/{Short_Hostname_Lowercase_var}/g' /ITS_tools/its-awx-server-build/awx-instance-deployment.yml"], shell=True, universal_newlines=True)

#Replace the Default Hostname in the base/awx.yaml file 
subprocess.check_output([f"sed -i 's/ITS-SYSVAR/{Short_Hostname_Lowercase_var}/g' /ITS_tools/its-awx-server-build/awx-on-k3s-main/base/awx.yaml"], shell=True, universal_newlines=True)
